...
export default class App extends React.Component {
  ...
  getWeather() {
    ...
    response = geocodeAndGetWeather(this.state.locationInputText);
    console.log(response.location);
    console.log(response.weather);
    this.setState({
      locationName: response.location,
      currentTemperature: Math.round(response.weather.current.apparentTemperature),
      currentSummary: response.weather.current.summary,
      currentIcon: response.weather.current.icon,
  ...
  render() {
    ...
    for (let i=0; i<5; i++) {
      let forecast = this.state.forecast[i];
      return (
        <View style={styles.container}>
          ...
          <View style={styles.forecastDay} key={i}>
            <Text style={styles.forecastIcon}><Icon size={30} name={icon(forecast[i].icon)} /></Text>
            <Text style={styles.forecastTemperature}>{Math.round(forecast[i].temperatureHigh)}</Text>
          </View>
        </View>
      );
    };
